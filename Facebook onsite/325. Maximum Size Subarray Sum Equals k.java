...
// 我们可以这样做,创建一个map,计算出数组中每个位置的数字和它之前的所有数字的和,并以它为key,相应位置为value放到map中,接下来对于遍历到的数组中的每个位置,我们要分成两种情况:

// 1.如果当前位置i和它之前的所有数字的和为k,那么子数组的长度即为i+1

// 2.如果不为k,此时需要看sum-k是否在map中,因为sum-(sum-k)=k,如果在map中,那么子数组的长度为i-map.get(sum-k);如果不在,那么把当前的sum和i放到map中,继续往下遍历。
